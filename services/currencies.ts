...
import {CurrenciesList, CurrenciesDetails, CurrenciesQuery, CurrencyCategory} from "@/types/currencies"
...
export const fetchAllCategories = async():Promise<CurrencyCategory[]> => {
    const {data} = await axios .get(ApiEndPoints.CATEGORIES_LIST)
    return data
}